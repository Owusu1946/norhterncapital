...
export async function sendReportEmail(to: string, subject: string, body: string, attachment: { filename: string, content: Buffer }) {
  const transporter = nodemailer.createTransport({
    host: process.env.SMTP_HOST,
    port: Number(process.env.SMTP_PORT || 587),
    secure: Number(process.env.SMTP_PORT) === 465,
  ...
  await transporter.sendMail({
    ...
    html: `
      <div style="font-family: sans-serif; padding: 20px; color: #333;">
        <h2 style="color: #01a4ff;">Hotel Performance Report</h2>
        <p>${body}</p>
        <p style="margin-top: 20px; font-size: 12px; color: #666;">
          This is an automated report generated by your AI Assistant at Northern Capital Hotel.
        </p>
      </div>
    `,
    attachments: [
